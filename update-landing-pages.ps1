# Batch update all electrician-*.html landing pages with popup functionality

$files = Get-ChildItem -Filter "electrician-*.html"
...
foreach ($file in $files) {
    ...
    # Update "Visit Main Site" button to be "Get Free Quote"
    $content = $content -replace '<a href="/" class="btn" style="background:rgba\(255,255,255,.08\);color:var\(--text\)">Visit Main Site</a>', '<a href="#" class="btn" style="background:rgba(255,255,255,.08);color:var(--text)" onclick="openQuotePopup(); return false;">Get Free Quote</a>'
    
    Set-Content -Path $file.FullName -Value $content -NoNewline
}

Write-Host "Done! Updated $($files.Count) landing pages."
